...
用 Google 試算表加 Apps Script 當免費的賽事資料來源(API 加自動爬蟲),作為 Cloudflare Worker 之外的備援方案。
...
| 賽事自動更新 | 每日 cron 加 KV | 試算表加選用每日觸發器 |
...
兩支腳本讀寫同一張試算表,放在同一個容器繫結(container-bound)專案的兩個檔即可:

| 檔(Apps Script 內) | 來源 | 角色 |
|---|---|---|
| `Code.gs` | [gas-api-script.js](gas-api-script.js) | API:`doGet` 把試算表內容回傳 JSON 給前端 |
| `Crawler.gs` | [gas-crawler-script.js](gas-crawler-script.js) | 爬蟲:選單 `syncRaces` 自動抓運動筆記與馬拉松世界寫入試算表 |

注意:兩檔原本都有 `formatDate`,同專案會「重複宣告」報錯。爬蟲端已改名 `formatDate_`,故現在可安全同放一個專案。
...
A–H 為前端必要欄;I/J 為選填。爬蟲會自動填 A–D、I、J,不覆寫你手填的 E–H(Strava/GPX)。
...
3. 貼上兩支腳本:
   ...
   - 用「＋ → 指令碼」新增 `Crawler.gs`,貼上 [gas-crawler-script.js](gas-crawler-script.js) 全文。
   - 儲存,回試算表重新整理(讓 `onOpen` 建立選單)。
4. 部署 Web App(供前端讀取):Apps Script「部署 → 新增部署作業 → 類型:Web 應用程式」,執行身分選「我」、誰可以存取選「任何人」,部署後複製 `/exec` 網址。
5. 把網址設給 App:
   - 執行時:在 App 的系統設定「後端 URL」貼上該 `/exec` 網址;或
   - 建置時:設 GitHub Actions Variable `GAS_API_URL`,build 會烘進 bundle。
   - 不需手改 `constants/index.ts`。

## 用爬蟲自動更新賽事

- 手動:試算表上方選單「🏃‍♂️ 賽事助手 → 🔄 從運動筆記與馬拉松世界更新賽事」(此為腳本建立的選單,名稱含 emoji)。以「日期_名稱」去重、只加未來賽事、append-only,不覆寫手填欄。
- 自動(選用,等同 Worker cron):Apps Script 左側「觸發條件 → 新增觸發條件」,函式選 `syncRaces`、事件來源選「時間驅動」、設每日一次。

## 過期賽事

`doGet` 在回傳時就濾掉過去日期的賽事(前端只會看到未來賽事);試算表的列不會被刪除,你手填的 Strava/GPX 與歷史紀錄都保留。

## 疑難排解

- 解析 0 筆或抓不到:來源網站常改版。看 Apps Script「執行紀錄」印出的 HTML 內容樣本,比對 [worker/src/lib.js](../worker/src/lib.js) 的 `parseBijiRaces` / `parseMwRaces`(與本爬蟲解析邏輯一致)調整 regex。
